...
object CreateMySqlPool {
  ...
  def apply(mysqlUrl:String,mysqlUser:String,mysqlPassword:String): GenericObjectPool[MySqlProxy] = {
    if (this.genericObjectPool == null) {
      this.synchronized {
        val jdbcUrl = mysqlUrl
        val jdbcUser = mysqlUser
        val jdbcPassword = mysqlPassword
        val size = ConfigManager.getPoolSize
