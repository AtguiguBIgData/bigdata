...
public class RecommenderService {
    ...
    private List<Recommendation> findContentBasedMoreLikeThisRecommendations(int mid, int maxItems) {
        MoreLikeThisQueryBuilder query = QueryBuilders.moreLikeThisQuery(/*new String[]{"name", "descri", "genres", "actors", "directors", "tags"},*/
                new MoreLikeThisQueryBuilder.Item[]{new MoreLikeThisQueryBuilder.Item(ConfigManager.getEsIndexName(), Constants.ES_TYPE, String.valueOf(mid))});
    ...
    private List<Recommendation> findContentBasedSearchRecommendations(String text, int maxItems) {
        ...
        return parseESResponse(esClient.prepareSearch().setIndices(ConfigManager.getEsIndexName()).setTypes(Constants.ES_TYPE).setQuery(query).setSize(maxItems).execute().actionGet());
    }
    ...
    public List<Recommendation> getHotRecommendations(HotRecommendationRequest request) {
        ...
        recs = jdbcTemplate.query("select * from " + Constants.DB_RATE_MORE_RECENTLY_MOVIES + " ORDER BY count DESC limit " + request.getSum(), new RowMapper<Recommendation>() {
            @Override
    ...
    public List<Recommendation> getContentBasedGenresRecommendations(SearchRecommendationRequest request) {
        List<Recommendation> recs = null;
        recs = jdbcTemplate.query("select * from " + Constants.DB_MOVIE + " where locate('"+ request.getText() +"',genres) > 0  limit 50", new RowMapper<Recommendation>() {
            @Override
            public Recommendation mapRow(ResultSet resultSet, int i) throws SQLException {
                Recommendation rec = new Recommendation();
                rec.setMid(resultSet.getInt("mid"));
    ...
    public List<Recommendation> getTopGenresRecommendations(TopGenresRecommendationRequest request) {
        List<Recommendation> recs = null;
        recs = jdbcTemplate.query("select * from " + Constants.DB_GENRES_TOP_MOVIES + " where gen='" + request.getGenres()+"' ORDER BY rank limit "+request.getSum(), new RowMapper<Recommendation>() {
            @Override
            public Recommendation mapRow(ResultSet resultSet, int i) throws SQLException {
                Recommendation rec = new Recommendation();
                rec.setMid(resultSet.getInt("mid"));
