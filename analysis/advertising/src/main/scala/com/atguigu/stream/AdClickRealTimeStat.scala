...
object AdClickRealTimeStat {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topics = ConfigManager.getkafkaAdTopic()
    val zookeeper = ConfigManager.getZookeeper

    // kafka消费者配置
    val adRealTimeLogDStream = createKafkaDStream(ssc,broker_list,topics,zookeeper)
    ...
    adRealTimeValueDStream.foreachRDD{ rdd =>
      // 更新Zookeeper中的偏移值
      val updateTopicDirs = new ZKGroupTopicDirs("advertise",topics)
  ...
  /**
    ...
    * @return
    */
  def createKafkaDStream(ssc: StreamingContext, brokers:String, topics:String, zookeeper:String) ={
    ...
    val topicDirs = new ZKGroupTopicDirs("advertise",topics)
