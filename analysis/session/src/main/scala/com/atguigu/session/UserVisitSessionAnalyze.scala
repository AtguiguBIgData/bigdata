...
object UserVisitSessionAnalyze {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val jsonStr = ConfigManager.getTaskJson
  ...
  /**
    * 业务功能四：获取top10活跃session 第二种算法[优化]
    *
    * @param taskid
    */

  def getTop10Session2(spark: SparkSession, taskid: String, top10CategoryList: Array[(CategorySortKey, String)], sessionid2ActionRDD: RDD[(String, UserVisitAction)]): Unit ={
    val categroyidArray = top10CategoryList.map(_._2).toArray

    //广播
    val categroyidArrayBrocast = spark.sparkContext.broadcast(categroyidArray)

    // MAP side JOIN
    val filtedActionRDD = sessionid2ActionRDD.filter{ case (sessionid,userVisitAction) =>
      var success = false
    ...
    val sss = groupCategroyid2Array.flatMap{ case (categroyid,items) =>
      ...
      items.toArray.sortBy(_._2).reverse.take(10).map{ case (sessionid,count) =>
        Top10Session(taskid,categroyid.toLong,sessionid,count)
      }
    ...
    sss.toDF.write
      .format("jdbc")
      .option("url", ConfigManager.getMySqlUrl)
      .option("dbtable", Constants.DB_TOP10SESSION)
      .option("user", ConfigManager.getMySqlUsername)
      .option("password", ConfigManager.getMySqlPassword)
  ...
  def getTop10Session(spark: SparkSession, taskid: String, top10CategoryList: Array[(CategorySortKey, String)], sessionid2ActionRDD: RDD[(String, UserVisitAction)]) {
    ...
    sessionDetailRDD.toDF().write
      .format("jdbc")
      .option("url", ConfigManager.getMySqlUrl)
      .option("dbtable", Constants.DB_SESSION_DETAIL)
      .option("user", ConfigManager.getMySqlUsername)
      .option("password", ConfigManager.getMySqlPassword)
      .mode(SaveMode.Append)
      .save()
  ...
  /**
    * 业务需求三：获取top10热门品类  第二种算法[优化]
    * @param taskid
    * @param spark
    * @param filtedUserVisitActionRDD
    */
  def getTop10Category2(taskid:String,spark: SparkSession,filtedUserVisitActionRDD:RDD[(String,UserVisitAction)]): Unit ={
    ...
    val categoryAction2countRDD = filtedUserVisitActionRDD.map{ case (sessionid, userVisitAction) =>
      ...
      }else if(userVisitAction.pay_category_ids != ""){
        categoryAction = (userVisitAction.pay_category_ids+"_pay",1L)
      }else if(userVisitAction.order_category_ids != ""){
        categoryAction = (userVisitAction.order_category_ids+"_order",1L)
      }
    ...
    val top10CategoryArray = top10Array.map{case (categroySortKey,categroyid) =>
      Top10Category(taskid,categroyid.toLong, categroySortKey.clickCount,categroySortKey.orderCount,categroySortKey.payCount)
    }
    ...
    top10DF.write
      .format("jdbc")
      .option("url", ConfigManager.getMySqlUrl)
      .option("dbtable", Constants.DB_TOP10CATEGORY)
      .option("user", ConfigManager.getMySqlUsername)
      .option("password", ConfigManager.getMySqlPassword)
      .mode(SaveMode.Append)
      .save()
  ...
  def randomExtractSession(spark: SparkSession, taskUUID: String, sessionid2AggrInfoRDD: RDD[(String, String)], sessionid2actionRDD: RDD[(String, UserVisitAction)]) {
    ...
    sessionRandomExtract.toDF().write
      .format("jdbc")
      .option("url", ConfigManager.getMySqlUrl)
      .option("dbtable", Constants.DB_SESSION_RANDOM_EXTRACT)
      .option("user", ConfigManager.getMySqlUsername)
      .option("password", ConfigManager.getMySqlPassword)
      .mode(SaveMode.Append)
      .save()
  ...
  def calculateAndPersistAggrStat(spark: SparkSession, value: mutable.HashMap[String, Int], taskUUID: String) {
    ...
    sessionAggrStatRDD.toDF().write
      .format("jdbc")
      .option("url", ConfigManager.getMySqlUrl)
      .option("dbtable", Constants.DB_SESSION_AGGR_STAT)
      .option("user", ConfigManager.getMySqlUsername)
      .option("password", ConfigManager.getMySqlPassword)
      .mode(SaveMode.Append)
      .save()
